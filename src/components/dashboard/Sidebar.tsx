...
const userNavItems: NavItem[] = [
  ...
  { href: "/dashboard/bookings", label: "My Bookings", icon: "confirmation_number" },
  { href: "/dashboard/visits", label: "Scheduled Visits", icon: "calendar_month" },
  { href: "/dashboard/saved", label: "Saved Listings", icon: "favorite_border" },
  { href: "/dashboard/messages", label: "Messages", icon: "chat_bubble_outline", badge: "2" },
  { href: "/dashboard/profile", label: "My Profile", icon: "person" },
];

const sellerNavItems: NavItem[] = [
  { href: "/dashboard/seller", label: "Overview", icon: "dashboard", exact: true },
  { href: "/dashboard/seller/listings", label: "My Listings", icon: "home_work" },
  { href: "/dashboard/seller/listings/new", label: "Add Listing", icon: "add_home" },
  { href: "/dashboard/seller/bookings", label: "Bookings", icon: "confirmation_number" },
  { href: "/dashboard/messages", label: "Messages", icon: "chat_bubble_outline", badge: "2" },
];

const agentNavItems: NavItem[] = [
  { href: "/dashboard/agent", label: "Overview", icon: "dashboard", exact: true },
  { href: "/dashboard/agent/listings", label: "My Listings", icon: "home_work" },
  { href: "/dashboard/agent/clients", label: "Clients", icon: "group" },
  { href: "/dashboard/agent/bookings", label: "Bookings", icon: "confirmation_number" },
  { href: "/dashboard/messages", label: "Messages", icon: "chat_bubble_outline", badge: "2" },
];

const adminNavItems: NavItem[] = [
  { href: "/dashboard/admin", label: "Overview", icon: "dashboard", exact: true },
  { href: "/dashboard/admin/users", label: "Manage Users", icon: "manage_accounts" },
  { href: "/dashboard/admin/listings", label: "All Listings", icon: "home_work" },
  { href: "/dashboard/admin/bookings", label: "All Bookings", icon: "confirmation_number" },
...
const roleMeta: Record<string, { label: string; subtitle: string; color: string }> = {
  user: { label: "Al-Diyar", subtitle: "My Dashboard", color: "text-white/50" },
  seller: { label: "Al-Diyar", subtitle: "Seller Portal", color: "text-accent" },
  agent: { label: "Al-Diyar", subtitle: "Agent Portal", color: "text-emerald-400" },
...
export default function Sidebar() {
  ...
  const isPending = (role === "seller" || role === "agent") && approvalStatus === "pending";
    ...
    <aside className="hidden md:flex flex-col w-64 bg-primary text-white shrink-0 h-full">
      ...
      {isPending && (
        <div className="mx-4 mt-4 rounded-xl bg-amber-500/20 border border-amber-400/30 px-4 py-3 flex items-start gap-2">
          <span className="material-icons-outlined text-amber-400 text-base mt-0.5">schedule</span>
          <div>
            <p className="text-xs text-amber-200 leading-snug font-medium">Pending Admin Approval</p>
            <p className="text-[11px] text-amber-300/70 mt-0.5">Access limited until approved</p>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/5">
          ...
          {session?.user?.image ? (
            ...
            <div className="h-8 w-8 rounded-full border border-white/20 bg-white/10 flex items-center justify-center shrink-0">
              <span className="material-icons-outlined text-sm text-white">person</span>
            </div>
          )}
