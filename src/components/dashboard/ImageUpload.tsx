...
const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (nonImageFiles.length > 0) {
      alert(
        `The following files are not images and will be skipped: ${nonImageFiles.map((f) => f.name).join(", ")}`,
      );
          ...
          {value.map((item, index) => {
            ...
            return (
              <div
                ...
                className="relative group aspect-square rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm bg-slate-100 dark:bg-slate-800"
              >
                <Image
                  fill
                  src={url}
                ...
                {typeof item !== "string" && (
                  <div className="absolute bottom-2 left-2 px-2 py-0.5 bg-primary/90 text-[10px] text-white font-bold rounded-full z-10 shadow-sm">
                    READY
                  </div>
                )}
                ...
                <button
                  ...
                  className="absolute top-2 right-2 p-1 bg-danger/90 hover:bg-danger text-white rounded-full shadow-lg transition-all transform hover:scale-110 z-20"
                  title="Remove image"
                >
                  <span className="material-icons-outlined text-sm block">
                    close
                  </span>
                </button>
              </div>
        ...
        className={`group relative w-full py-8 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed transition-all duration-300 shadow-sm overflow-hidden
          ${
            value.length >= 10
              ...
              : "border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30 hover:border-primary hover:bg-slate-100 dark:hover:bg-slate-800/50"
          }`}
      >
        ...
        <div className="relative z-10 flex flex-col items-center">
          <div
            ...
            ${
              value.length >= 10
                ...
                : "bg-slate-100 dark:bg-slate-800 group-hover:bg-primary/10 group-hover:ring-primary/5"
            }`}
          >
            <span
              className={`material-icons-outlined text-4xl transition-colors
              ${value.length >= 10 ? "text-slate-400" : "text-text-muted group-hover:text-primary"}
            `}
            >
              {value.length >= 10 ? "block" : "add_photo_alternate"}
            </span>
          </div>
          <p
            className={`text-sm font-bold transition-colors
            ${value.length >= 10 ? "text-slate-400" : "text-text-main group-hover:text-primary"}
          `}
          >
            {value.length >= 10
          ...
          <p className="text-[11px] text-text-muted mt-1 font-medium italic">
            {value.length >= 10
      ...
      <div className="flex items-start gap-2 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/50 mt-2">
        ...
        <div>
          ...
          <p className="text-[11px] text-text-muted leading-relaxed">
            While choosing files, hold <b>Ctrl</b> (Windows) or <b>Command</b>{" "}
            (Mac) to select multiple photos at once. Or simply click and drag to
