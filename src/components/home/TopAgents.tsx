...
const TopAgents = () => {
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        const res = await fetch("/api/agents?limit=5&sort=new");
    ...
    <section className="py-24 bg-white dark:bg-slate-950 transition-colors">
      <div className="max-w-10/12 mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-center mb-16">
          ...
          <Link
            href="/agents"
            className="hidden md:block text-xs font-bold text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-800 px-6 py-2.5 rounded-full uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors duration-300"
          >
            See more
          </Link>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5].map((i) => (
              <div
                key={i}
                className="bg-slate-100 dark:bg-slate-900 rounded-4xl h-[400px] animate-pulse"
              />
            ))}
          </div>
        ) : agents.length > 0 ? (
