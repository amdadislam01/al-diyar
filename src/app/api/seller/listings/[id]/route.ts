...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        if (updates.status && !["Active", "Inactive", "Pending", "Sold"].includes(updates.status as string)) {
            return NextResponse.json({ message: "status must be 'Active', 'Inactive', 'Pending', or 'Sold'" }, { status: 400 });
        }
